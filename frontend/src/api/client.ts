...
export async function searchPlayers(
  ...
  const response = await fetch(`/api/players?q=${encodeURIComponent(query)}`, { signal });
...
export async function comparePlayers(
  ...
  return readJson<ComparisonResponse>(await fetch(`/api/compare?${params.toString()}`, { signal }));
}
